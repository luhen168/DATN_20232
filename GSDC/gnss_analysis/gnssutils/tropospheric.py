...
latDegreeToLambdaAvgMap = [2.77,3.15,2.75,1.81,1.55]
...
class Troposphere():
    class DryAndWetMappingValues:
        def __init__(self,dryMappingValue,wetMappingValue):
            self.dryMappingValue = dryMappingValue
            self.wetMappingValue = wetMappingValue
    class DryAndWetZenithDelays:
        def __init__(self,dryZenithDelay,wetZenithDelay):
            self.dryZenithDelaySec = dryZenithDelay
            self.wetZenithDelaySec = wetZenithDelay
        def __repr__(self):
    ...
    def interpolate(self,point1X,point1Y,point2X,point2Y,xOutput):
        ...
        if math.fabs(deltaX) > MINIMUN_INTERPOLATION_THRESHOLD:
    ...
    def calculateZenithDryAndWetDelaysSec(self,userLatitudeRadians,heightMetersAboveSeaLevel,dayOfYear1To366):
        absLatitudeDeg = math.degrees(math.fabs(userLatitudeRadians))
        ...
        zenithWetDelayAtSeaLeverSeconds =  (((1.0e-6 * K2 * RD) / (GM * (Lambda + 1.0) - beta * RD)) * (waterVaporPressureMbar / tempKelvin))
    ...
    def calculateTropoCorrectionMeters(self,satElevationRadians,userLatitudeRadian,heighMeterAboveSeaLevel,utcTimeMilies):
        ...
        dayOfYear_1To366 = utctimemilis_to_day_of_year(utcTimeMilies[0])
