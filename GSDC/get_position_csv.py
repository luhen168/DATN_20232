...
path = 'gnss_2024_06_12_21_09_sat.csv'
...
def main():
    ...
    gnss = pd.read_csv(path, dtype={'SignalType': str})
    ...
    df = pd.DataFrame(position_array, columns=["utcTimeMillis", "Latitude", "Longitude"])
    time_get_file = path.split('_', 1)[1].rsplit('.', 1)[0]  # Get the part after the first underscore and before the extension
    new_output_file_name = f"position_data_{time_get_file}.csv"
